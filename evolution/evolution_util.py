...
def get_training_dataset(include_loops=True):
    linearized_qubo_list, qubo_list, energy_list, \
        solution_list, problem_list, edge_index_list, edge_weight_list = get_linearized_qubos(include_loops=include_loops)
...
def get_linearized_qubos(include_loops=True):
    qubo_list, problem_list = get_problem_qubos()
    linarized_qubo_list = []
    energy_list = []
    solution_List = []
    edge_index_list = []
...
def get_fitness_value(linearized_approx_list, qubo_list, min_energy_list, fitness_parameters, min_approx=0):
    ...
    for linearized_approx, qubo, min_energy in zip(linearized_approx_list, qubo_list, min_energy_list):
        solution_quality, true_approx = get_quality_of_approxed_qubo(linearized_approx, qubo, min_energy)
        ...
        fitness = (a * (1 - solution_quality) +
                   b * (1 - np.square(d - approx_quality)) +
                   c * np.floor(1-solution_quality))
        #print('Qubo:', qubo)
        #print('Non-Zero:', get_nonzero_count(linearize_qubo(qubo)))
        if not true_approx > (min_approx * get_nonzero_count(linearize_qubo(qubo))):
            fitness = 0
        fitness_list.append(fitness)
    return np.mean(fitness_list)


def aggregate_saved_problems():
    database = engine.get_database()
    aggregation_array = []
    for i, (_, metadata) in enumerate(database.iter_metadata()):
        if i == 0:
            aggregation_array = prepare_aggregation_array(metadata.qubo_size)
...
def prepare_aggregation_array(qubo_size):
    aggregation_array = [[], []]
...
def check_model_config_fit(model_descr):
    parameter_list = model_descr.split("_")
    model_problem = parameter_list[1]
    model_size = int(parameter_list[2])
    ...
    return model_problem == problem and model_size == qubo_size  #, fitness_params
